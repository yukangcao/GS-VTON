...
def get_mask_location(model_type, category, model_parse: Image.Image, keypoint: dict, width=384,height=512):
    ...
    if category == 'dresses':
        ...
        # parse_mask = (parse_array == label_map["left_shoe"]).astype(np.float32) + (parse_array == label_map["right_shoe"]).astype(np.float32)
        parser_mask_fixed_lower_cloth = (parse_array == label_map["skirt"]).astype(np.float32) + \
                                        (parse_array == label_map["pants"]).astype(np.float32)
        # parser_mask_fixed_lower_cloth = (parse_array == label_map["left_shoe"]).astype(np.float32) + \
        #                                 (parse_array == label_map["right_shoe"]).astype(np.float32)
