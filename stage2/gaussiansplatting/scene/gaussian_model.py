...
class GaussianModel:
    ...
    def __init__(
        ...
        self.anchor = {}
        self.localize = False
        self.semantic_label=torch.empty(0)
    ...
    def resetColor(self):
        rgb=np.ones((self._xyz.shape[0],3))
        ...
        features[:, 3:, 1:] = 0.0
        # self._features_dc[self.mask] = features[:, :, 0:1][self.mask].transpose(1, 2).contiguous()
        # self._features_rest[self.mask]= features[:, :, 1:][self.mask].transpose(1, 2).contiguous()
        
        self._features_dc = nn.Parameter(
           features[:, :, 0:1][self.mask].transpose(1, 2).contiguous().requires_grad_(True)
        )
        self._features_rest = nn.Parameter(
            features[:, :, 1:][self.mask].transpose(1, 2).contiguous().requires_grad_(True)
        )        
    ...
    def training_setup(self, training_args):
        ...
        l = [
            ...
            },
            {
                ...
                # "lr": training_args.feature_lr / 2.0,
                "name": "f_rest",
            },
            {
    ...
    def load_ply(self, path):
        ...
        self.semantic_label=nn.Parameter(torch.zeros_like(self._opacity, dtype=torch.float, device="cuda").requires_grad_(True))
