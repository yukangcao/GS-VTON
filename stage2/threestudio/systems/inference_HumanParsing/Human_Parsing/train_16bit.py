...
def get_arguments():
    ...
    parser.add_argument("--num-worker", type=int, default=8,
                        help='num-worker < or = < !cat /proc/cpuinfo | grep processor | wc -l >')
...
def log2wandb(lr, loss, project, epoch, data_dir, log_dir):
    ...
    def log(weight='./{log_dir}/model_parsing_best.pth.tar', mode='Best', data_dir=None):
        ...
        for _, value in mIoU.items():
            values.append(value)
        with open(f"./{log_dir}/log_{mode}.csv", "a") as outfile:
    ...
    log(weight=f'./{log_dir}/model_parsing_best.pth.tar', mode='Best', data_dir=data_dir)
    log(weight=f'./{log_dir}/checkpoint_last.pth.tar', mode='Last', data_dir=data_dir)
...
def init_log_csv(log_dir):
    ...
    mode = ['Best', 'Last']
    for i in mode:
        with open(f"./{log_dir}/log_{i}.csv", "a") as outfile:
            csvwriter = csv.writer(outfile)
...
def main():
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if (epoch + 1) % (args.eval_epochs) == 0:
            ...
            # save best
            result_metrics = eval(model_checkpoint=f'./{args.log_dir}/checkpoint_last.pth.tar', data_dir=args.data_dir)
        ...
        if (epoch + 1) >= args.schp_start and (epoch + 1 - args.schp_start) % args.cycle_epochs == 0:
            ...
            schp.moving_average(schp_model, model, 1.0 / (
                    cycle_n + 1))  # <=== cập nhật param, cycle_n càng lớn thì param của model schp càng nhiều hơn model gốc
            cycle_n += 1
            schp.bn_re_estimate(train_loader,
                                schp_model)  # <=== Dùng weight vừa được cập nhật ở trên để training train_loader
